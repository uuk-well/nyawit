...
export function PengeluaranForms({
  item,
  kebunList,
}: {
  item?: Item;
  ...
  const [state, formAction] = useActionState<ActionState | undefined, FormData>(
    isEdit ? updatePengeluaran : createPengeluaran,
    undefined,
  );
  ...
  if (kebunList.length === 0 && !isEdit) {
    return (
      <p className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
        Buat kebun terlebih dahulu di menu Kebun sebelum mencatat pengeluaran.
      </p>
    );
  }
...
export function DeletePengeluaranButton({ id }: { id: string }) {
